...
export class AwesomeLoader extends BlitElement 
{
    ...
    {
        return html`
        <div class="grid grid-rows-4 h-full gap-4 relative">
            <div class="row-start-2 relative w-[min(25vh,80vw-2rem)] mx-auto">
                <div style="background-image: repeating-conic-gradient(#539ffd 0 12deg,#0576f8 12deg 24deg);"
                     class="absolute bottom-0 rounded-full w-inherit aspect-square animate-spin-slow">
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 733 733"
                     class="absolute bottom-0 p-4 rounded-full w-inherit hover:scale-105 ease-in-out duration-300">
            ...
            <div class="row-start-3 font-bold text-4xl text-center italic">loading...</div>
        </div>
    `;
    }
}
